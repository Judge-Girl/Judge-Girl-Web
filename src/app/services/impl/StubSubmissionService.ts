...
import {Judge, JUDGE_STATUSES, VerdictIssuedEvent, JudgeStatus, Submission, CodeFile} from '../../models';

@Injectable(
  {providedIn: 'root'}
)
// TODO migrate to judge-girl M2
export class StubSubmissionService extends SubmissionService {
  ...
  private schedulingSubject = new Subject<VerdictIssuedEvent>();
  ...
  private async scheduleSubmission(submission: Submission) {
    setTimeout(async () => {
      ...
      this.schedulingSubject.next(new VerdictIssuedEvent(
        problem.id, problem.title, submission.id, submission.verdict));
    }, 400);
  }
